...
global_params = []
plot_x = []
for idx in range(1,8200):
    if idx % 100 == 0:
        plot_x.append(idx)
        file_name = f'trajs/f{idx:d}.lmp'
        pmn = read(file_name, format='lammps-data', style='atomic')
        ...
        print(f"[Processed] {idx:d} → global_param = {global_param:.4f}")


## Plot the order parameter
plt.figure(figsize=(6,4))
plt.plot(plot_x, global_params,
         linestyle='-',
         linewidth=2,
         color='black',
         alpha=1.0,
...
plt.tight_layout()
plt.savefig('order_parameter_L12.png', dpi=300)
plt.close()
