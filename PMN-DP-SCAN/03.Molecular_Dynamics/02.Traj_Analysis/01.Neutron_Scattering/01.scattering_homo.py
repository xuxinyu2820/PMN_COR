...
if __name__ == '__main__':
    ...
    if os.path.exists('pmn_{}K_scattering_function_homo.npy'.format(temp)):
        data = np.load('pmn_{}K_scattering_function_homo.npy'.format(temp))
        q_vector = data[0]
        scattering_function_avg = data[1]
    else:
        ## Trajectory
        traj_path = "../../01.MD_Simulation/NVT/{:d}K_homo/pmn_{}K_500000steps.traj".format(temp, temp)
        ...
        for frame_idx, atoms in enumerate(traj[10::10]):
        ...
        np.save('pmn_{}K_scattering_function_homo.npy'.format(temp), data)
    ...
    plt.tight_layout()
    plt.savefig('pmn_{}K_scattering_with_SM_homo.png'.format(temp), dpi=300, transparent=True)
    plt.close()
    ...
    # plt.tight_layout()
    # plt.savefig('pmn_{}K_scattering_function_homo.png'.format(temp))
